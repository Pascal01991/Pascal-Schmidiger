# Prüfung 2

## Bedingungen

🕐 Eine Stunde Zeit

## Aufsetzen

1. `LK2` in Visual Studio Code öffnen.
1. `npm install` ausführen.
1. `Ctrl + Shift + P` und `Developer: Reload Windows` ausführen, um die Testumgebung einzuschalten.
1. Tests ausführen.

## Lösen

⛔️ Viele Tests schlagen fehl
😓 Eure Aufgabe?
✅ Möglichst viele Tests zum Laufen bringen
💡 Anpassungen in `lk2.library.js` und `lk2.test.js` möglich
⚠️ *Test-Ergebnisse* nicht anpassen
⚠️ *Erwartungen* (`expect()`) nicht anpassen
💡 Nicht Überdenken (manchmal ist die Lösung gar nicht so komplex)

## Fertigstellen

💾 Änderungen speichern (commit)
⬆️ Nach GitHub hochladen (push)